...
bounded : forall {n} (i : Fin n) -> toℕ i ℕ< n
bounded zero    = s≤s z≤n
bounded (suc i) = s≤s (bounded i)

prop-toℕ-≤ : forall {n} (x : Fin n) -> toℕ x ℕ≤ N.pred n
prop-toℕ-≤ zero                 = z≤n
prop-toℕ-≤ (suc {n = zero}  ())
prop-toℕ-≤ (suc {n = suc n} i)  = s≤s (prop-toℕ-≤ i)

nℕ-ℕi≤n : forall n i -> n ℕ-ℕ i ℕ≤ n
...
_+′_ : forall {m n} (i : Fin m) (j : Fin n) -> Fin (N.pred m ℕ+ n)
