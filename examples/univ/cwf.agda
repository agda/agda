...
castElem {Γ}{A}{B} (eqTy B=A) (elem u) = elem (ΓB=ΓA << u)
  where
    ...
    ΓB=ΓA = eqS < refS , Bx=Acx >
      where
        postulate Bx=Acx : (x : El Γ) -> B ! x =S A ! (refS << x)
--         Bx=Acx x =
--           chain> B ! x
--              === A ! x            by B=A x
...
_//_ {Γ}{Δ}{A} (elem t) σ'@(el < σ , pσ >) =
    elem (el < tσ , (\{x}{y} -> prf x y) >)
  where
    tσ : (x : El Δ) -> El (A ! σ x)
    tσ x = t # σ x

    postulate prf : (x y : El Δ)(x=y : x == y) -> t # σ x == _ << t # σ y
--     prf x y x=y =
--       chain> t # σ x
...
wk {Γ}{A} = el < f , (\{x}{y} -> pf x y) >
  where
    ...
    postulate pf : (x y : El (Γ , A))(x=y : x == y) -> f x == _ << f y
  ...
  where
    ...
    f x = el < g , (\{x}{y} -> pg) >
      where
        ...
        postulate pg : {y z : El (A ! x)}(y=z : y == z) -> g y == _ << g z
--         pg {y}{z} y=z =
--           chain> u # el < x , y >
  ...
  where
    ...
    f x = p u << y
      where
        ...
        postulate p : (u : El (pi Γ A)) -> (B / [ elem u ]) ! x =S B ! el < x , u # x >
--         p (el < u , pu >) = pFam B (
...
--           where open module C9 = Chain _==_ (ref {_}) (trans {_})
    ...
    postulate pf : {x y : El Γ}(x=y : x == y) -> f x == _ << f y
--     pf {x}{y} x=y =
--       chain> q1 << (w # x) # (u # x)
--          === q1 << (q3 << w # y) ## (u # x)
