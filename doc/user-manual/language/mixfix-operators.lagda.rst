...
For instance, we can join the name parts ``if``, ``then``,
and ``else`` with underscores into a single name ``if_then_else_``,
...
..
  ::
  module Applications (A : Set) (x : Bool) (y z : A) where
...
For the following discussion of precedence, assume the following operators::
...
Depending on which of ``_and_`` and ``_⇒_`` is given precendence,
...
An operator with only interior holes ``_`` is called closed,
