...
$AGDA --interaction --trace-imports --color=never <<EOF
...
$AGDA --interaction --trace-imports=3 --color=never <<EOF
IOTCM "Issue6261.agda" None Indirect (Cmd_load "Issue6261.agda" [])
EOF

# Should not print anything in the presence of -v0
$AGDA --interaction -v0 --trace-imports=3 --color=never <<EOF
IOTCM "Issue6261.agda" None Indirect (Cmd_load "Issue6261.agda" [])
EOF

# Should not print anything with --trace-imports=0
$AGDA --interaction --trace-imports=0 --color=never <<EOF
IOTCM "Issue6261.agda" None Indirect (Cmd_load "Issue6261.agda" [])
EOF
