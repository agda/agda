...
import Agda.Interaction.Imports ( typeCheckMain, Mode(TypeCheck), sourceInfo, CheckResult(CheckResult), crInterface )
...
mainTCM = do
  ...
  CheckResult { crInterface = i } <- typeCheckMain (TypeCheck RegularInteraction) =<< sourceInfo f
  compilerMain i
