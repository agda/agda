...
import Internal.Utils.NonemptyList ()
...
prop_cluster_complete :: Fun Int (NonemptyList C) -> [Int] -> Bool
prop_cluster_complete (Fun _ f) as =
  (`all` cluster f as) $ \ cl ->
  (`all` cl) $ \ a ->
  let csa = toList $ f a in
  let cl' = delete a $ toList cl in
...
prop_cluster_permutation :: Fun Int (NonemptyList C) -> [Int] -> Bool
prop_cluster_permutation (Fun _ f) as =
  sort as == sort (concatMap toList (cluster f as))

prop_cluster_single :: a -> [a] -> Bool
prop_cluster_single a as =
  isSingleton $ cluster (const (0 :! [])) $ (a:as)

prop_cluster_idem :: Fun a (NonemptyList C) -> a -> [a] -> Bool
prop_cluster_idem (Fun _ f) a as =
  isSingleton $ cluster f $ toList $ head $ cluster f (a:as)
