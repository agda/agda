...
--
--   import File000 using ()
--   ...
--   import File(NNN-1) using ()
--
...
createMainContent n = unlines $ concat
  [ map (\ j -> unwords [ "import", filePrefix ++ print0d w j, "using", "()" ]) [0..n-1]
  ]
  where
  w = length $ show n
...
createContent n i = unlines $ concat
  ...
  , map (\ j -> unwords [ "import", filePrefix ++ print0d w j, "using", "()" ]) [0..i-1]
  , [ ""
    , "data D : Set where c : D"
    , ""
    , "test : D"
