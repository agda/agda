...
(defcustom agda2-backend-default-cmd
  nil
  "The default command for Agda backends (nil to ask every time)."
  :type 'string
  :group 'agda2)
...
(defun agda2-backend-cmd (backend cmd)
  "Run the current backend's (from variable `agda2-backend`) interaction command."
  (interactive (list (agda2-read-backend) (or agda2-backend-default-cmd (read-string "Command:"))))
  (if (agda2-goal-at (point))
     (agda2-goal-cmd "Cmd_backend_hole" nil 'goal nil backend (agda2-string-quote cmd))
     (agda2-go 'save t 'busy t "Cmd_backend_top" backend (agda2-string-quote cmd))
  )
)
