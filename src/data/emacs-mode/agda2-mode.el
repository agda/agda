...
(defun agda2--map-identifiers (func)
  "Call FUNC on every identifier in the current buffer.
FUNC is invoked with two arguments, a string representing the
identifier and a the Xref info, a cons-call (FILENAME . POINT) as
generated by Agda.  If no identifiers were found, the function
will return nil.  Otherwise it returns t."
  (save-excursion
    (let (start end)
      (goto-char (point-min))
      (catch 'done
        (unless (next-single-property-change (point) 'agda2-xref-info)
          (throw 'done nil))
        (while t
          ...
          (let ((xinfo (get-text-property (1- (point)) 'agda2-xref-info)))
            (when (< start end)
              (let ((ident (buffer-substring-no-properties start end)))
                (unless (string-match-p "_" ident) ;avoid mixfix confusion
                  (funcall func ident xinfo))))))))))

(defvar agda2-last-ident-table (make-hash-table :test 'equal)
  "Cached table of identifiers and their locations.")

(defun agda2--update-last-ident-table (&optional keep)
  "Update `agda2-last-ident-table'.
If KEEP is non-nil, do not reset `agda2-last-ident-table' before
searching the buffer."
  (save-excursion
    (unless keep
      (clrhash agda2-last-ident-table))
    (agda2--map-identifiers
     (lambda (ident xinfo)
       (unless (member xinfo (gethash ident agda2-last-ident-table))
         (push xinfo (gethash ident agda2-last-ident-table)))))))

(defun agda2--make-xref-here ()
  "Generate an `xref-item' at the current point in the current buffer."
  (xref-make
   (concat
...
(defun agda2--make-xref (xinfo)
  "Create an `xref-item' from XINFO.
XINFO is a cons-cell (FILENAME . POINT), as generated by Agda."
  (with-current-buffer (find-file-noselect (car xinfo))
    (goto-char (cdr xinfo))
    (agda2--make-xref-here)))

(defun agda2--xref-<= (x1 x2)
  "Return t if X1 should be sorted before X2.
X1 and X2 are both `xref-item' objects as generated by
`agda2--make-xref'.  We therefore know that their
`xref-item-location's are specifically `xref-buffer-location'."
  (let* ((l1 (xref-item-location x1)) (l2 (xref-item-location x2))
         (b1 (buffer-name (xref-buffer-location-buffer l1)))
         (b2 (buffer-name (xref-buffer-location-buffer l2))))
    (and (or (string< b1 b2) (string= b1 b2))
         (<= (xref-buffer-location-position l1)
             (xref-buffer-location-position l2)))))

(cl-defmethod xref-backend-identifier-completion-table ((_ (eql 'agda2)))
  "Generate a list of possible candidates."
  (agda2--update-last-ident-table)
  agda2-last-ident-table)

(defun agda2-find-xinfo (ident)
  "Return a list of possible locations for IDENT.
Each element of the list is a cons-cell of the form (FILENAME
. POINT), as generated by Agda"
  (cond
   ((let ((xinfo (get-text-property 0 'agda2-xref-info ident)))
      (and xinfo (list xinfo))))
   ((progn
      (agda2--update-last-ident-table)
      (gethash ident agda2-last-ident-table)))))

(cl-defmethod xref-backend-definitions ((_ (eql 'agda2)) ident)
  "Generate a list of definitions for identifier IDENT."
  (mapcar #'agda2--make-xref (agda2-find-xinfo ident)))

(defun agda2-buffer-list ()
  "Return a list of all buffers deriving from `agda2-mode'."
  (let (bufs)
...
(cl-defmethod xref-backend-references ((_ (eql 'agda2)) ident)
  "Generate a list of references for IDENT."
  (let ((xinfos (agda2-find-xinfo ident)) xrefs)
    (dolist (buf (agda2-buffer-list))
      (with-current-buffer buf
        (agda2--map-identifiers
         (lambda (_ident xinfo)
           (when (member xinfo xinfos)
             (push (agda2--make-xref-here) xrefs))))))
    (sort xrefs #'agda2--xref-<=)))
...
(cl-defmethod xref-backend-apropos ((_ (eql 'agda2)) pattern)
  ...
  (dolist (buf (agda2-buffer-list))
    (with-current-buffer buf
      (agda2--update-last-ident-table t)))
  (let ((regexp (xref-apropos-regexp pattern)) xrefs)
    (maphash
     (lambda (ident refs)
       (when (string-match-p regexp ident)
         (dolist (xinfo refs)
           (push (agda2--make-xref xinfo) xrefs))))
     agda2-last-ident-table)
    (sort xrefs #'agda2--xref-<=)))
