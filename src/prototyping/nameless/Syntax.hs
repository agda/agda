...
newtype Scope = Scope { unScope :: Term }
  deriving (Show)

abstract :: Name -> Term -> Scope
abstract me e = Scope $ letmeB 0 e
  where
    ...
    letmeB this (Lam (Scope e))    = Lam (Scope $ letmeB (this + 1) e)
    letmeB this (Pi e1 (Scope e2)) = Pi (letmeB this e1)
                                        (Scope $ letmeB (this + 1) e2)
...
instantiate what (Scope body) = what'sB 0 body
  where
    ...
    what'sB this (Lam (Scope body))  = Lam (Scope $ what'sB (this + 1) body)
    what'sB this (Pi e (Scope body)) = Pi (what'sB this e)
                                          (Scope $ what'sB (this + 1) body)
...
piView :: MonadPlus m => Agency (Term -> m (Binding, Term))
