...
(defun eri-current-line-empty nil
  "Returns t if the current line is empty (not counting white space)."
  (equal (current-indentation)
         (eri-current-line-length)))

(defun eri-maximum (xs)
  "Calculates maximum element in XS. Returns nil if the list is
empty."
  (if xs (apply 'max xs)))
...
(defun eri-split (x xs)
  "Returns a pair of lists, (xs1, xs2). If XS is sorted, then XS =
 (append xs1 xs2), and all elements in xs1 are <= X, whereas all elements
in xs2 are > X."
...
(defun eri-calculate-indentation-points-on-line (max)
  "Calculates indentation points on current line. Only points left
of column number MAX are included. If MAX is nil, then all points are
included. Returns points in ascending order.
...
(defun eri-new-indentation-point ()
  "Calculates a new indentation point, two steps in from the
indentation of the first non-empty line above the current line. If
there is no such line 2 is returned."
  (save-excursion
    (while
        (progn
          (forward-line -1)
...
(defun eri-calculate-indentation-points (reverse)
  "Calculates some indentation points. Gives them in reverse order if
...
(defun eri-indent (&optional reverse)
  "Cycle between some possible indentation points. With prefix
argument, cycle in reverse order.
