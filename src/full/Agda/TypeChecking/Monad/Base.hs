...
data PreScopeState = PreScopeState
  ...
  , stPreBoundModuleNames   :: !(HashMap ModuleName IsBound)
    -- ^ Information about which module names from the current module
    -- that are bound (declared in let declarations).
...
initPreScopeState = PreScopeState
  ...
  , stPreBoundModuleNames     = HMap.empty
...
stBoundModuleNames :: Lens' (HashMap ModuleName IsBound) TCState
stBoundModuleNames f s =
  f (stPreBoundModuleNames (stPreScopeState s)) <&>
  \x -> s {stPreScopeState = (stPreScopeState s) {stPreBoundModuleNames = x}}
