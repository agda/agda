...
-- András: 2023-09-23: TODO: Control.Monad.Writer is a major performance bug
-- and should be purged from all code.
...
instance (Monoid w, ReadTCState m) => ReadTCState (Leaky.WriterT w m)
...
instance (Monoid w, MonadReduce m) => MonadReduce (Leaky.WriterT w m)
...
instance (Monoid w, MonadTCEnv m) => MonadTCEnv (Leaky.WriterT w m)
...
instance (Monoid w, MonadTCState m) => MonadTCState (Leaky.WriterT w m)
...
instance (Monoid w, MonadTCM tcm) => MonadTCM (Leaky.WriterT w tcm)
