...
instance Binary Agda.Syntax.Fixity.Fixity where
...
instance (Binary a) => Binary (Agda.Syntax.Common.Arg a) where
  put (Arg a b) = put a >> put b
  get = {-# SCC "get<Arg>" #-} get >>= \a -> get >>= \b -> return (Arg a b)

instance Binary Agda.Syntax.Common.Hiding where
...
instance Binary Agda.Syntax.Internal.Type where
  put (El a b) = put a >> put b
  get = {-# SCC "get<Type>" #-} get >>= \a -> get >>= \b -> return (El a b)

instance Binary Agda.Syntax.Internal.MetaId where
  put (MetaId a) = put a
  get = {-# SCC "get<MetaId>" #-} get >>= \a -> return (MetaId a)

instance (Binary a) => Binary (Agda.Syntax.Internal.Blocked a) where
  put (Blocked a b) = put a >> put b
  get = {-# SCC "get<Blocked>" #-} get >>= \a -> get >>= \b -> return (Blocked a b)

instance (Binary a) => Binary (Agda.Syntax.Internal.Abs a) where
  put (Abs a b) = put a >> put b
  get = {-# SCC "get<Abs>" #-} get >>= \a -> get >>= \b -> return (Abs a b)

instance Binary Agda.Syntax.Internal.Term where
...
instance Binary Agda.Syntax.Internal.Sort where
...
instance Binary Agda.Syntax.Literal.Literal where
...
instance Binary Agda.Syntax.Common.IsAbstract where
...
instance Binary Agda.Syntax.Internal.Pattern where
