...
currentInterfaceVersion = 20250724 * 10 + 0
...
encode a = do
    ...
    newD@Dict{..} <- liftIO $ emptyDict collectStats
    root     <- liftIO $ (`runReaderT` newD) $ icode a
    nodeL    <- benchSort $ l nodeD
    stringL  <- benchSort $ l stringD
    lTextL   <- benchSort $ l lTextD
    sTextL   <- benchSort $ l sTextD
    integerL <- benchSort $ l integerD
    naturalL <- benchSort $ l naturalD
    doubleL  <- benchSort $ l doubleD

    -- Record reuse statistics.
    whenProfile Profile.Sharing $ do
      statistics "pointers" termC
    whenProfile Profile.Serialize $ do
      statistics "Integer"     integerC
      statistics "Natural"     naturalC
      statistics "Lazy Text"   lTextC
      statistics "Strict Text" sTextC
      statistics "String"      stringC
      statistics "Double"      doubleC
      statistics "Node"        nodeC
      statistics "Shared Term" termC
    ...
    bits1 <- Bench.billTo [ Bench.Serialization, Bench.BinaryEncode ] $
      return $!! B.encode (root, nodeL, stringL, lTextL, sTextL, integerL, naturalL, doubleL)
...
decode s = do
  ...
  res <- liftIO $ E.handle (\(E.ErrorCall s) -> pure $ Left s) $ do
    ((r, nodeL, stringL, lTextL, sTextL, integerL, naturalL, doubleL), s, _) <- return $ runGetState B.get s 0
    let ar = unListLike
