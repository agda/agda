...
solveSizeConstraints flag =  do
  ...
  constrainedMetas <- Set.unions <$> do
    forM  (ccs) $ \ (cs :: List1 ProblemConstraint) -> do
      ...
      enterClosure (Fold.maximumBy (compare `on` (length . envContext . tcContext .  clEnv)) $ fmap theConstraint cs) $ \ _ -> do
...
castConstraintToCurrentContext' cl = do
  let modN  = envCurrentModule $ coldEnv $ clEnv cl
      delta = envContext $ tcContext $ clEnv cl
...
castConstraintToCurrentContext c = do
  ...
  let cl = theConstraint c
      cp = envCurrentCheckpoint $ tcContext $ clEnv cl
  sigma <- caseMaybeM (viewTC $ eCheckpoints . key cp)
          (do
            ...
            let delta = envContext $ tcContext $ clEnv cl
...
-}
  ...
  solved <- fmap Set.unions $ forM (Map.assocs $ theSolution sol) $ \ (m, a) -> do
    ...
    ifM (isFrozen m `or2M` (not <$> asksTC (envAssignMetas . metaEnv))) (return Set.empty) $ do
    ...
    and <$> do
      forM (Set.toList ms) $ \ m -> do
        ...
        let no = do
              ...
              return False
        ifM (isFrozen m `or2M` do not <$> asksTC (envAssignMetas . metaEnv)) no $ {-else-} do
          reportSDoc "tc.size.solve" 20 $
            "solution " <+> prettyTCM (MetaV m []) <+>
            " := "      <+> prettyTCM inf
          t <- metaType m
...
computeSizeConstraint c = do
  let cxt = envContext $ tcContext $ clEnv $ theConstraint c
