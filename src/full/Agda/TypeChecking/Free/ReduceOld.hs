...
module Agda.TypeChecking.Free.ReduceOld
  ( ForceNotFree
  , forceNotFree
  , reallyFree
  , IsFree(..)
...
import Agda.TypeChecking.Free.ReduceNew (IsFree(..))
...
forceNotFree :: (ForceNotFree a, Reduce a)
...
--   blocked on a meta.
reallyFree :: (Reduce a, ForceNotFree a)
...
nonFreeVars xs = VarSet.fromList $ mapMaybe takeNotFree $ IntMap.toAscList xs
  where
    takeNotFree :: (Int, IsFree) -> Maybe Int
    takeNotFree (!n, NotFree) = Just n
    takeNotFree _ = Nothing

-- Return the set of variables for which there is still hope that they
-- may not occur.
varsToForceNotFree :: (MonadFreeRed m) => m VarSet
varsToForceNotFree = gets nonFreeVars
...
instance (Reduce a, ForceNotFree a) => ForceNotFree (Abs a) where
  ...
  forceNotFree' a@Abs{} = do
    -- traceM "DIVEEEEEEEEEEEEEEE"
    ...
    res <- reduceIfFreeVars (bracket_ (modify $ IntMap.mapKeys succ) (\ _ -> modify $ IntMap.mapKeys pred) .
...
instance ForceNotFree Term where
  forceNotFree' = \case
    Var x es   -> do
      ...
      -- traceShowM x
      -- traceShowM metas
      case IntMap.lookup x map of
        Nothing -> Var x <$> forceNotFree' es
        Just entry  -> do
          -- traceM ("METASSAS OLD" ++ " " ++ show entry)
          -- traceShowM x
          -- traceShowM metas
          put $ IntMap.adjust (const $ MaybeFree metas) x map
