...
--   not @Nothing@.
clauseToRewriteRule :: QName -> QName -> I.Clause -> Maybe RewriteRule
...
instance ToNLPat (Arg I.DeBruijnPattern) (Elim' NLPat) where
  toNLPat (Arg ai p) = case p of
    VarP _ x        -> app $ PVar (I.dbPatVarIndex x) []
    DotP _ u        -> app $ PTerm u
    ConP c _ ps     -> app $ PDef (conName c) $ toNLPat ps
    LitP l          -> app $ PTerm $ I.Lit l
    ProjP o f       -> Proj o f
...
instance ToNLPat (NamedArg I.DeBruijnPattern) (Elim' NLPat) where
