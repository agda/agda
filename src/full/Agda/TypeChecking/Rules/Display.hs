...
patternToTerm p ret =
  case p of
    ...
    A.ConP _ (AmbQ [c]) ps -> pappToTerm c (Con (ConHead c Inductive []) ConPCon) ps ret
...
exprToTerm e =
  case unScope e of
    ...
    A.Def f  -> pure $ Def f []
    A.Con (AmbQ (c:_)) -> pure $ Con (ConHead c Inductive []) ConPCon [] -- Don't care too much about ambiguity here
    A.Lit l -> pure $ Lit l
