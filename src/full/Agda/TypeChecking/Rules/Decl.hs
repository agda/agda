...
checkDecl d = setCurrentRange d $ do
    ...
    checkMaybeAbstractly :: forall m i a . ( MonadTCEnv m , AnyIsAbstract i, AnyIsOpaque i, MonadDebug m )
                         => i -> m a -> m a
    checkMaybeAbstractly i =
      let a = case anyIsAbstract i of
            AbstractDef -> inAbstractMode
            ConcreteDef -> inConcreteMode
          o = case anyIsOpaque i of
            OneOpaque i        -> inOpaqueMode i
            NoOpaque           -> inTransparentMode
            InconsistentOpaque ->
              -- Amy, 2023-01-17, impossible because:
              --
              -- a) checkMaybeAbstractly is only ever called with
              -- DefInfo or [DefInfo]
              --
              -- b) checkMaybeAbstractly is only ever called with more
              -- than one DefInfo in the unquote cases
              --
              -- c) the unquote cases have DefInfos generated by
              -- ConcreteToAbstract to have the OpaqueId duplicated from
              -- that of the unquote itself, so they are all the same
              const $ __IMPOSSIBLE_VERBOSE__ "Block with inconsistent opaques"
      in a . o
...
revisitRecordPatternTranslation qs = do
  ...
  classify q = inIdentifierReductionMode q $ \ def -> do
    case theDef def of
...
checkInjectivity_ names = Bench.billTo [Bench.Injectivity] $ do
  ...
  Fold.forM_ names $ \ q -> inIdentifierReductionMode q $ \ def -> do
...
-- | Freeze metas created by given computation if in abstract or opaque mode.
whenAbstractFreezeMetasAfter :: A.DefInfo -> TCM a -> TCM a
whenAbstractFreezeMetasAfter Info.DefInfo{ defAccess, defReduces } m = do
  if isReducible defReduces  then m else do
...
checkAxiom' gentel kind i info0 mp x e = whenAbstractFreezeMetasAfter i $ defaultOpenLevelsToZero $ do
  ...
  let defn = defaultDefn info x t lang $
        case kind of   -- #4833: set abstract already here so it can be inherited by with functions
          ...
          _         -> __IMPOSSIBLE__
        where fun = FunctionDefn funD{ _funAbstr = Info.defReduces i }
...
checkPrimitive i x (Arg info e) =
    ...
    addConstant' x info x t $
        Primitive { primAbstr    = Info.defReduces i
                  , primName     = s
                  , primClauses  = []
...
checkTypeSignature' gtel (A.Axiom funSig i info mp x e) =
  ...
  Bench.billTo [Bench.Typing, Bench.TypeSig] $
    checkAxiom' gtel funSig i info mp x e
