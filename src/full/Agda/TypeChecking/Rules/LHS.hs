...
checkLeftHandSide :: forall a.
  ...
  -> [ProblemEq]
     -- ^ Equations coming from a clause telescope (used by clauses generated by reflection).
  -> (LHSResult -> TCM a)
     -- ^ Continuation.
  -> TCM a
checkLeftHandSide call f ps a withSub' strippedPats telEqs =
 ...
 traceCallCPS call $ \ ret -> do
  ...
  let finalChecks :: LHSState a -> TCM a
      finalChecks (LHSState delta qs0 (Problem eqs rps _) b psplit) = do
        ...
        -- Add equations from the clause telescope here (and not in the initial LHSState since they
        -- live in the final lhs context). The linearity check will ensure that the types and de
        -- Bruijn indices from the telescope matches those computed by the lhs checker.
        eqs <- addContext delta $ checkPatternLinearity (telEqs ++ eqs)
