...
instance ToConcrete A.Expr where
    ...
    toConcrete (A.RecWhere i es ass) = bracket appBrackets do
      ...
      Fold.for_ ass \case
        ...
        _                             -> pure ()
      C.RecWhere (getRange i) . concat . List1.toList <$> toConcrete es

    toConcrete (A.RecUpdateWhere i e0 es ass) = bracket appBrackets do
      -- InternalToAbstract generates FieldAssignments of the following
      -- form to associate the fresh 'A.Name's generated for the record
      -- field bindings should be printed back without disambiguators
      Fold.for_ ass \case
        ...
        _                             -> pure ()
      e0 <- toConcrete e0
      C.RecUpdateWhere (getRange i) e0 . concat . List1.toList <$> toConcrete es
