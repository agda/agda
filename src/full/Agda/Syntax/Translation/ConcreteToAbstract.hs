...
import Data.Foldable (traverse_, foldrM, Foldable (toList, fold))
import Data.Set (Set)
import Data.Map (Map)
  ...
  where
    ...
    makeBinding d = do
      ...
      setCurrentRange r $ case d of
        ...
        C.NiceUnfolding{}     -> __IMPOSSIBLE__
...
instance ToAbstract NiceDeclaration where
  ...
  -- Record definitions (mucho interesting)
    ...
    -- UnfoldingId unfoldings are generated by the nicifier when
    -- encountering 'where' clauses, as a way of preserving the
    -- provenance of the outer 'abstract' block. The semantics is that
    -- id (the where clause) becomes an alias for id'.
    NiceUnfolding _ id (UnfoldingId id') -> do
      modifyTCLens stUnfoldDefs $ \c ->
        let outer = Map.lookup id' c
            m' :: Maybe (HashSet I.QName) -> Maybe (HashSet I.QName)
            m' Nothing = outer
            m' (Just c) = Just (c <> fold outer)
        in Map.alter m' id c
      pure []
      ...
      let
        ...
        close c xs = do
          ...
          fmap (HashSet.insert qname . (<> xs)) $ case Map.lookup qname qname_abs of
            Just id -> do
              reportSDoc "scope.unfold" 30 $ "qname" <+> pure (pretty qname) <+> "is associated with aid" <+> text (show id)
              pure $ fromMaybe __IMPOSSIBLE__ $ Map.lookup id abs_unfold
            Nothing -> pure mempty
      let uf = fold $ Map.lookup id abs_unfold
      closed <- foldrM close uf xs
      reportSDoc "scope.unfold" 30 $ "unfolding id" <+> text (show id) <+> ":" <+> pure (pretty (toList closed))
...
addToUnfold aid qn =
  do
    ...
    modifyTCLens stAbstractBlocks $ Map.alter k2 qn
  where
    k1 Nothing = Just (HashSet.singleton qn)
