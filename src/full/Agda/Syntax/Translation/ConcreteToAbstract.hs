...
instance ToAbstract NiceDeclaration where
  ...
  -- Record definitions (mucho interesting)
    ...
    -- NiceUnfoldings with an underlying UnfoldingId are generated by
    -- the nicifier when encountering 'where' clauses, as a way of
    -- preserving the provenance of the outer 'abstract' block. The
    -- semantics is that id (the where clause) becomes an alias for id'.
    ...
    -- Otherwise we're looking at the NiceDecl
    --   @id = unfolding (qnames...)@,
    -- which is generated by the nicifier so that we may know which
    -- names to associate with the given abstract block. What we must
