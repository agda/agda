...
data NiceDeclaration
  ...
  | RecDef Range Fixity' IsAbstract Name (Maybe (Ranged Induction)) (Maybe (ThingWithFixity Name)) [LamBinding] PositivityCheck [NiceDeclaration]
...
declKind (RecDef _ _ _ x _ _ pars pc _)  = LoneDef (RecName pc $ parameters pars) x
        ...
        LoneSig k x -> do
          ...
          let prefix :: [NiceDeclaration] -> [NiceDeclaration]
              prefix = case (d, ds0) of
                (NiceRecSig{}, [r@(RecDef _ _ _ _ _ _ _ True _)]) -> ([d, r] ++)
                _                                                 ->
...
--          trace ("mFixity = " ++ show mFixity) $
    ...
    mkAbstract d =
      case d of
        ...
        RecDef r f a x i c ps pc cs      -> (\ a -> RecDef r f a x i c ps pc) <$> setAbstract a <*> mapM mkAbstract cs
...
notSoNiceDeclaration d =
  case d of
    ...
    RecDef r _ _ x i c bs _ ds       -> Record r x i (unThing <$> c) bs Nothing $ map notSoNiceDeclaration ds
