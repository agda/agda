          ...
          case sols of
           Just (cls : _) -> withInteractionId ii $ do
            ...
            case cls' of
             ...
             Right cls' -> do
              cls'' <- forM cls' $ \ (I.Clause _ _ tel ps body t catchall
                                        exact recursive reachable ell wm
                                        _) -> do
                withCurrentModule (AN.qnameModule def) $ do
                 -- Normalise the dot patterns
                 ps <- addContext tel $ normalise ps
                 body <- etaContract body
                 fmap modifyAbstractClause $ inTopContext $ reify $
                   AN.QNamed def $
                   I.Clause noRange noRange tel ps body t catchall exact
                     recursive reachable ell wm False
