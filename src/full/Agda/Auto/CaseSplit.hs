...
caseSplitSearch :: forall o . IORef Int -> Int -> [ConstRef o] -> Int -> Int -> ConstRef o -> CSCtx o -> MExp o -> [CSPat o] -> IO [Sol o]
caseSplitSearch ticks nsolwanted chints depthinterval depth recdef ctx tt pats = do
   ...
   dobody = do
    case findperm (map snd (drophid ctx)) of
     Just perm -> do
      ...
      let reccelt = HI NotHidden (NoId, cdtype recdefd)
          ctx'' = ctx' ++ [reccelt]
        ...
        where
         ...
         dobranches (con : cons) = do
          ...
          let ff t = case rm t of
                        Pi _ h _ it (Abs _ ot) ->
                         let (xs, inft) = ff ot
                         in ((Pair h (scrut + length xs), lift (scrut + length xs + 1) it) : xs, inft)
                        _ -> ([], lift scrut t)
              (newvars, inftype) = ff (cdtype cond)
              constrapp = mm $ App Nothing (mm OKVal) (Const con) (foldl (\xs (Pair h v, _) -> mm $ ALCons h (mm $ App Nothing (mm OKVal) (Var v) (mm ALNil)) xs) (mm ALNil) (reverse newvars))
              pconstrapp = CSPatConApp con (map (\(Pair hid v, _) -> HI hid (CSPatVar v)) newvars)
              ...
              ctx1 = map (\(HI hid (id, t)) -> HI hid (id, thesub t)) (take scrut ctx) ++
                     reverse (map (\((Pair hid _, t), i) -> HI hid (Id (newvarprefix ++ show i), t)) (zip newvars [0..])) ++
          ...
          case unifyexp inftype scruttype' of
           Nothing -> do
            ...
            if res then do -- branch absurd
              dobranches cons
             else do -- branch dont know
              return []
           Just unif ->
            do
             let (ctx2, tt2, pats2) = removevar ctx1 tt' pats' unif
             sols <- rc (depth - costCaseSplit) (length ctx - 1 - scrut) ctx2 tt2 pats2
             case sols of
              [] -> return []
              _ -> do
...
renamec :: Nat -> ConstRef o -> MExp o -> MExp o
renamec rv rc = r 0
 where
  r j e =
   case rm e of
         App uid ok elr args -> case elr of
          Var v | v - j == rv -> mm $ App uid ok (Const rc) (rs j args)
          _ -> mm $ App uid ok elr (rs j args)
         Lam hid (Abs mid e) -> mm $ Lam hid (Abs mid (r (j + 1) e))
         Pi uid hid possdep it (Abs mid ot) -> mm $ Pi uid hid possdep (r j it) (Abs mid (r (j + 1) ot))
         Sort{} -> e
...
localTerminationEnv pats =
 ...
 in g 0 pats
localTerminationSidecond :: ([Nat], Nat, [Nat]) -> Nat -> MExp o -> EE (MyPB o)
localTerminationSidecond (is, size, vars) reccallvar b =
  ok reccallvar b
 where
     ok rvar e = mmpcase (False, prioNo, Nothing) e $ \e -> case e of
      App _ _ elr args -> mpret $ Sidecondition
       (oks rvar args)
       (case elr of
          Var v | v == rvar -> if size == 0 then mpret (Error "localTerminationSidecond: no size to decrement") else okcall 0 size vars args
          _ -> mpret OK
       )
      Lam _ (Abs _ e) -> ok (rvar + 1) e
      Pi _ _ _ it (Abs _ ot) -> mpret $ Sidecondition
       (ok rvar it)
       (ok (rvar + 1) ot)
      Sort{} -> mpret OK
      AbsurdLambda{} -> mpret OK
     oks rvar as = mmpcase (False, prioNo, Nothing) as $ \as -> case as of
      ALNil -> mpret OK
      ALCons _ a as -> mpret $ Sidecondition
       (ok rvar a)
       (oks rvar as)
      ALConPar as -> oks rvar as
