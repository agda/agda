...
--
-- Time complexity: /O(e log n)/, if the lists are not inspected.
-- Inspection of a prefix of a list is linear in the length of the
-- prefix.
  ...
  bfs !q !map = case SQ.lview q of
    ...
    Just ((u, p), q) ->
      ...
      then bfs q map
               ...
               (Map.insert u (BQ.size p, BQ.toList p) map)
...
--
-- Time complexity (assuming that the predicates take constant time to
-- compute): /O(e log n)/.
