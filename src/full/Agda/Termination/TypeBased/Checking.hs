...
sizeCheckTerm' expected t@(Con ch ci elims) = do
  ...
  case constraints of
    [x] -> do
      -- Similarly to inductive record projections, coinductive constructors provide us with a level of guardedness, which can be unilized by the termination checker.
      -- The following example illustrates the necessity of this behavior. Assume that _,_ is a constructor of the coinductive type Stream:
      --
      -- foo : Stream Nat
      -- foo = 0 , foo
      --
      -- This definition is valid, since it is productive, but there is no rigid size variable from LHS that can prove the termination.
      -- In fact, this rigid size variable is generated by the invocation of constructor.
      isCoinductive <- liftTCM $ isCoinductiveConstructor constructorName
      when isCoinductive $ do
