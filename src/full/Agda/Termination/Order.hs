...
module Agda.Termination.Order
  ...
  , supremum, infimum
  , orderSemiring
  , le, leBase, lt, unknown
  , nonIncreasing, decreasing, isDecr, isImp
  , NotWorse(..)
  , isOrder
  ) where
...
data Order
  = Decr !Bool !Bool {-# UNPACK #-} !Int
    -- ^ Decrease of callee argument wrt. caller parameter.
    --
    --   The first @Bool@ indicates whether the decrease (if any) is usable.
  ...
  | Imp
    -- ^ The chain of calls is impossible (e.g. a decrease from a base
    -- constructor)
...
instance NotWorse Order where
  o1 `notWorse` o2 = case comparable o1 o2 of
    POLT  -> False
    POLE  -> False
    POEQ  -> True
    POAny -> True
...
setUsability u o = case o of
  Decr _ b k -> Decr u b k
  Unknown    -> o
  Imp        -> o

toBase :: Order -> Bool
toBase (Decr _ b _) = b
toBase Unknown      = False
toBase Imp          = True
...
decr :: (?cutoff :: CutOff) => Bool -> Bool -> Int -> Order
decr u b k = case ?cutoff of
...
isOrder (Decr _ _ k) = withinCutOff k
isOrder Unknown      = True
isOrder Imp          = True

-- | @le@, @lt@, @decreasing@, @unknown@: for backwards compatibility, and for external use.
le :: Order
le = Decr False False 0

-- | LHS is structurally minimal
leBase :: Order
leBase = Decr False True 0

-- | Usable decrease.
lt :: Order
lt = Decr True False 1

unknown :: Order
unknown = Unknown

nonIncreasing :: Order -> Bool
nonIncreasing (Decr _ _ k) = k >= 0
nonIncreasing Unknown      = False
nonIncreasing Imp          = True

-- | Decreasing and usable?
decreasing :: Order -> Bool
decreasing (Decr u _ k) = u && k > 0
decreasing Unknown      = False
decreasing Imp          = True
...
(Decr u b k) .*. (Decr u' b' l)
  -- It is impossible to decrease a base constructor
  | b' && k > 0 = Imp
...
minO o1 o2 = case comparable o1 o2 of
  POLT  -> o1
  POLE  -> o1
  POEQ  -> o2
  POAny -> o2
