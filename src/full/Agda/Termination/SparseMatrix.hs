...
module Agda.Termination.SparseMatrix
  ( -- * Basic data types
    Matrix(Matrix)
  , unM  -- required by the internal test-suite
...
--  , Agda.Termination.Matrix.zipWith
  ...
  , add
  , intersectWith
  , interAssocWith  -- required by the internal test-suite
  , mul
  , transpose
  , Diagonal(..)
  , toSparseRows  -- required by the internal test-suite
  , supSize  -- required by the internal test-suite
    -- * Modifying matrices
  , addRow
  , addColumn
  ) where
