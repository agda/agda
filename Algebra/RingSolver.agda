...
------------------------------------------------------------------------
-- Simplified interface to the solver, based on metaprogramming

-- Private, since there is no support for metaprogramming in Agda yet.

private

  open import Meta

  record Polys (p₁ p₂ : Expr carrier) : Set where
    n     : ℕ
    ρ     : Vec carrier n
    p₁'   : Polynomial n
    p₂'   : Polynomial n
  ...
  postulate
    toPolys : (p₁ p₂ : Expr carrier) -> Polys p₁ p₂
    -- Traverse the structure of p₁ and p₂ and detect all uses of +,
    -- *, -, 0# and 1#. Put the other subtrees in the environment (ρ),
    -- making sure to use the same variable for identical subtrees.

  prove' :  (p₁ p₂ : Expr carrier)
         -> let ps = toPolys p₁ p₂ in
            ⟦ Polys.p₁' ps ⟧↓ Polys.ρ ps ≈
            ⟦ Polys.p₂' ps ⟧↓ Polys.ρ ps
         -> ($ p₁) ≈ ($ p₂)
  prove' p₁ p₂ eq = p₁-eq ⟨ trans ⟩ eq ⟨ trans ⟩ sym p₂-eq
    where
    open module P = Polys (toPolys p₁ p₂)

  -- Example use:
  --   prove' ⟦ (x + y) * z ⟧ ⟦ z * x + y * z ⟧ byDef
  -- Plus two applications of prop-left-inverse...
